...
interface AlertContextValue {
    showToast: (message: string, type?: ToastType, title?: string) => void;   // ← added title param
    ...
    return (
        <div
            className="
                ...
                sm:top-22 sm:bottom-auto sm:left-1/2 sm:-translate-x-1/2 sm:px-0 sm:w-max
            "
        >
            {toasts.map((t) => (
                <div
                    ...
                    `}
                >
                    {/* Icon — align to top when title+message stacked */}
                    <span className={t.title ? "self-start mt-0.5 shrink-0" : "shrink-0"}>
                        {TOAST_ICONS[t.type]}
                    </span>

                    {/* Text — single line OR title + subtitle */}
                    {t.title ? (
                        <div className="flex-1 min-w-0">
                            <p className="font-black text-sm leading-tight">{t.title}</p>
                            <p className="font-medium text-xs mt-0.5 leading-tight">{t.message}</p>
                        </div>
                    ) : (
                    ...
                    <button
                        ...
                        className="ml-1 p-1 rounded-lg hover:bg-black/10 transition-colors cursor-pointer self-start mt-0.5"
                        aria-label="Dismiss"
                    >
                        <X size={14} />
                    </button>
