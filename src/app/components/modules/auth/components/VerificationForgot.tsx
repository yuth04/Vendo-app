...
const VerificationForgort = () => {
  ...
  useEffect(() => {
    const pending = authService.getPendingResetEmail();
    if (!pending) {
      showNotification("Please request a password reset first.", "error");
  ...
  const handlePaste = (
    ...
    const pasted = e.clipboardData
      .getData("text")
      .replace(/\D/g, "")
      .slice(0, 6 - pasteIndex);
    if (!pasted) return;
    const updated = [...code];
    pasted.split("").forEach((char, i) => {
      updated[pasteIndex + i] = char;
    });
    setCode(updated);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response?.reset_token) {
        authService.setResetToken(response.reset_token);
      }
...
export default VerificationForgort;
