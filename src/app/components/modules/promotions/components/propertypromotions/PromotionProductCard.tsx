...
export default function PromotionProductCard({ discountId, product, amount, type, brandName, reviews, index }: Props) {
    ...
    const discountedPrice = parseFloat(product.discount_price);
    const savings         = originalPrice - discountedPrice;
    const discountPct     = type === "percent"
    ...
    return (
        <div
            key={`${discountId}-${product.id}-${index}`}
            className="flex flex-col group cursor-pointer rounded-[2rem] border-2 card-theme p-4 mb-4 transition-all duration-500 hover:shadow-md custom-main-color-border-hover bg-[var(--header-bg)]"
        >
