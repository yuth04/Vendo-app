...
interface WishlistApiError {
    message: string;
    code?: string | number;
}

export function useWishlistData<T>(
    fetchFn: (signal?: AbortSignal) => Promise<ApiResponse<T>>,
    defaultValue?: T,
    enableClientFetch: boolean = false,
) {
    const [data, setData] = useState<T | undefined>(defaultValue);
    const [error, setError] = useState<WishlistApiError | null>(null);
    ...
    useEffect(() => {
        ...
        fetchFn(controller.signal)
            .then((res) => {
                if (res.error) {
                    setError(res.error as WishlistApiError);
