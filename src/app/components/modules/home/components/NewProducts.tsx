...
const NewProducts = () => {
    ...
    const fetchNewProducts = useCallback(() => productClient.fetchNewProducts(), []) as () => Promise<any>;
    ...
    const { data: newProductsRes, loading: isNewProductsLoading } = useApiData<NewProductResponse>(fetchNewProducts, EMPTY_NEW_PRODUCTS_RES, true);
    ...
    const productsList = useMemo<NewProduct[]>(() => {
        return (newProductsRes as any)?.products ?? [];
    }, [newProductsRes]);
    ...
    const handleWishlistClick = async (e: React.MouseEvent, product: NewProduct, wishlisted: boolean) => {
        e.preventDefault();
        e.stopPropagation();
        if (user) {
            if (!wishlisted) {
    ...
    const loading = isNewProductsLoading || isDiscountLoading || isReviewLoading || isBrandsLoading;
    ...
    return (
        <section className="max-w-7xl mx-auto py-2 sm:py-5">
            ...
            {/* Products grid area layout wrapper */}
            {loading ? (
                ...
                <EmptyState
                    ...
                    description="The modern product inventory catalog is currently empty right now."
                    imageSrc={Notfound}
